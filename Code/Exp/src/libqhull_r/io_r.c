...
void qh_produce_outputt(qhT *qh, double **ext, int *half)
{
    int tempsize= qh_setsize(qh, qh->qhmem.tempstack);

    qh_prepare_output(qh);
    qh_produce_outputt2(qh, ext, half);
    if (qh_setsize(qh, qh->qhmem.tempstack) != tempsize) {
        qh_fprintf(qh, qh->ferr, 6206, "qhull internal error (qh_produce_output): temporary sets not empty(%d)\n",
                   qh_setsize(qh, qh->qhmem.tempstack));
        qh_errexit(qh, qh_ERRqhull, NULL, NULL);
...
void qh_produce_outputt2(qhT *qh, double **ext, int *half)
{
    ...
    for (i = 0; i < qh_PRINTEND; i++)
        qh_printfacets2(qh, qh->fout, qh->PRINTout[i], qh->facet_list, NULL, !qh_ALL, ext, half);
    fflush(NULL);
...
void qh_printafacet2(qhT *qh, FILE *fp, qh_PRINT format, facetT *facet, boolT printall,  double *ext)
{
    realT color[4], offset, dist, outerplane, innerplane;
    boolT zerodiv;
    coordT *point, *normp, *coordp, **pointp, *feasiblep;
...
void qh_printextremes2(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall, int *half) {
    setT *vertices, *points;
    pointT *point;
    vertexT *vertex, **vertexp;
    int id;
...
void qh_printextremes_2d2(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall, int *half) {
    int numfacets, numridges, totneighbors, numcoplanars, numsimplicial, numtricoplanars;
    setT *vertices;
    facetT *facet, *startfacet, *nextfacet;
    vertexT *vertexA, *vertexB;
    ...
    do {
        ...
        if (facet->visitid) {
            if (vertexA->visitid != qh->vertex_visit) {
                vertexA->visitid= qh->vertex_visit;
                //qh_fprintf(qh, fp, 9089, "%d\n", qh_pointid(qh, vertexA->point));
                half[indd] = qh_pointid(qh, vertexA->point);
                indd++;
            }
            if (vertexB->visitid != qh->vertex_visit) {
                vertexB->visitid= qh->vertex_visit;
                //qh_fprintf(qh, fp, 9090, "%d\n", qh_pointid(qh, vertexB->point));
                half[indd] = qh_pointid(qh, vertexB->point);
                indd++;
            }
        }
        facet->visitid= qh->visit_id;
...
void qh_printextremes_d2(qhT *qh, FILE *fp, facetT *facetlist, setT *facets, boolT printall, int *half) {
    setT *vertices;
    vertexT *vertex, **vertexp;
    boolT upperseen, lowerseen;
    facetT *neighbor, **neighborp;
    ...
    FOREACHvertex_(vertices) {
            ...
            {
                qh_fprintf(qh, fp, 9092, "%d\n", qh_pointid(qh, vertex->point));
                half[indd] = qh_pointid(qh, vertex->point);
                ++indd;
            }
        }
    qh_settempfree(qh, &vertices);
...
void qh_printfacets2(qhT *qh, FILE *fp, qh_PRINT format, facetT *facetlist, setT *facets, boolT printall, double **ext, int *half) {
    int numfacets, numsimplicial, numridges, totneighbors, numcoplanars, numtricoplanars;
    facetT *facet, **facetp;
    setT *vertices;
    coordT *center;
    ...
    if (format == qh_PRINTnone)
        ...
        if (qh->DELAUNAY)
            qh_printextremes_d2(qh, fp, facetlist, facets, printall, half);
        else if (qh->hull_dim == 2)
            qh_printextremes_2d2(qh, fp, facetlist, facets, printall, half);
        else
            qh_printextremes2(qh, fp, facetlist, facets, printall, half);
    }else if (format == qh_PRINToptions)
        qh_fprintf(qh, fp, 9187, "Options selected for Qhull %s:\n%s\n", qh_version, qh->qhull_options);
    else if (format == qh_PRINTpoints && !qh->VORONOI)
        qh_printpoints_out(qh, fp, facetlist, facets, printall);
        ...
        qh_printbegin(qh, fp, format, facetlist, facets, printall);
        FORALLfacet_(facetlist)
            qh_printafacet2(qh, fp, format, facet, printall, *ext++);
        FOREACHfacet_(facets)
            qh_printafacet2(qh, fp, format, facet, printall, *ext++);
        qh_printend(qh, fp, format, facetlist, facets, printall);
    }
